<#
================================================================================
    NAPELLAM BANK - WS01 WORKSTATION SETUP SCRIPT
================================================================================

PURPOSE:
    This script configures WS01 workstation and joins it to napellam.local domain.
...
Write-Host "  NAPELLAM BANK - WS01 WORKSTATION SETUP" -ForegroundColor Cyan
Write-Host "================================================================================" -ForegroundColor Cyan
Write-Host ""
...
# Check if ammulu.orsu is already local admin
$IsLocalAdminConfigured = $false
if ($IsDomainJoined) {
    try {
        $LocalAdmins = Get-LocalGroupMember -Group "Administrators" -ErrorAction SilentlyContinue
        $IsLocalAdminConfigured = ($LocalAdmins.Name -contains "NAPELLAM\ammulu.orsu")
    } catch {
        # Fallback check
        $output = net localgroup Administrators 2>$null
        $IsLocalAdminConfigured = ($output -match "ammulu.orsu")
    }
}
...
if (-not $IsDomainJoined) {
    ...
    if ($Adapter) {
        ...
        Write-Host "    [+] IP address set to: 192.168.100.20" -ForegroundColor Green
    ...
    if ($CurrentName -ne "WS01") {
        Write-Host "    [+] Renaming computer to WS01..." -ForegroundColor Yellow
        Rename-Computer -NewName "WS01" -Force | Out-Null
        Write-Host "    [+] Computer will be renamed to WS01" -ForegroundColor Green
    } else {
        Write-Host "    [+] Computer is already named WS01" -ForegroundColor Green
    }
    ...
    try {
        ...
        Write-Host "  1. Network configured: 192.168.100.20" -ForegroundColor White
        Write-Host "  2. Computer renamed to: WS01" -ForegroundColor White
...
if ($IsDomainJoined -and -not $IsLocalAdminConfigured) {
    ...
    Write-Host "[*] Adding ammulu.orsu to local Administrators group..." -ForegroundColor Yellow
    
    try {
        Add-LocalGroupMember -Group "Administrators" -Member "napellam\ammulu.orsu" -ErrorAction Stop
        Write-Host "    [+] ammulu.orsu added to local Administrators" -ForegroundColor Green
    } catch {
        if ($_.Exception.Message -like "*already a member*") {
            Write-Host "    [+] ammulu.orsu is already a local Administrator" -ForegroundColor Green
        } else {
            net localgroup Administrators napellam\ammulu.orsu /add 2>$null
    ...
    Write-Host "  Computer:      WS01.napellam.local" -ForegroundColor White
    Write-Host "  IP Address:    192.168.100.20" -ForegroundColor White
    Write-Host "  Domain:        napellam.local" -ForegroundColor White
    Write-Host "  Local Admin:   ammulu.orsu (Domain Admin)" -ForegroundColor White
    Write-Host ""
    Write-Host "NEXT STEPS:" -ForegroundColor Yellow
    Write-Host "  1. Log out of current session" -ForegroundColor White
    Write-Host "  2. Login as: napellam\ammulu.orsu" -ForegroundColor White
    Write-Host "  3. Password: princess" -ForegroundColor White
    Write-Host "  4. This caches Domain Admin credentials in memory!" -ForegroundColor White
    Write-Host ""
    Write-Host "WS01 is ready for pentesting!" -ForegroundColor Green
    Write-Host ""
}
...
if ($IsDomainJoined -and $IsLocalAdminConfigured) {
    Write-Host ">>> WS01 ALREADY CONFIGURED <<<" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host "  - Local admin: ammulu.orsu" -ForegroundColor White
    Write-Host ""
    Write-Host "You can login as: napellam\ammulu.orsu (password: princess)" -ForegroundColor Green
    Write-Host ""
}
